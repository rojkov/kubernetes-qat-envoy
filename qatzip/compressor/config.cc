...
unsigned int
hardwareBufferSizeEnum(qatzip::compressor::Qatzip_HardwareBufferSize hardware_buffer_size) {
  switch (hardware_buffer_size) {
  case qatzip::compressor::Qatzip_HardwareBufferSize::Qatzip_HardwareBufferSize_SZ_4K:
    return 4 * 1024;
  case qatzip::compressor::Qatzip_HardwareBufferSize::Qatzip_HardwareBufferSize_SZ_8K:
    return 8 * 1024;
  case qatzip::compressor::Qatzip_HardwareBufferSize::Qatzip_HardwareBufferSize_SZ_32K:
    return 32 * 1024;
  case qatzip::compressor::Qatzip_HardwareBufferSize::Qatzip_HardwareBufferSize_SZ_64K:
    return 64 * 1024;
  case qatzip::compressor::Qatzip_HardwareBufferSize::Qatzip_HardwareBufferSize_SZ_128K:
    return 128 * 1024;
  case qatzip::compressor::Qatzip_HardwareBufferSize::Qatzip_HardwareBufferSize_SZ_512K:
    return 512 * 1024;
  default:
    return 64 * 1024;
  }
...
Envoy::Compression::Compressor::CompressorPtr QatzipCompressorFactory::createCompressor() {
  return std::make_unique<QatzipCompressorImpl>(tls_slot_->getTyped<QatzipThreadLocal>().getSession());
}
...
QatzipCompressorLibraryFactory::createCompressorFactoryFromProtoTyped(
    const qatzip::compressor::Qatzip& proto_config, Server::Configuration::FactoryContext& context) {
